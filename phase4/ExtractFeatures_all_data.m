...
function SeperateEatingAndNonEatingAction(emg_file,imu_file,eating_frames_file,fps,last_ts,last_frame,output_path)
    ...
    for i = 1 : len
        ...
        if ~isempty(ea_emg) && ~isempty(ea_imu)
            Z1 = zscore(ea_emg(:,2:end),[ ],1);
            Z2 = zscore(ea_imu(:,2:end),[ ],1);
            countEa = countEa+1;
            dlmwrite(output_path+"EatingAction/EA.csv",(Z1)','delimiter',',','-append');
            dlmwrite(output_path+"/EatingAction/EA.csv",(Z2)','delimiter',',','-append');
        end
        if i == 1
        ...
        if ~isempty(nea_emg) && ~isempty(nea_imu)
            Z1 = zscore(nea_emg(:,2:end),[ ],1);
            Z2 = zscore(nea_imu(:,2:end),[ ],1);
            countNea = countNea+1;
            dlmwrite(output_path+"/NonEatingAction/NEA.csv",(Z1)','delimiter',',','-append');
            dlmwrite(output_path+"/NonEatingAction/NEA.csv",(Z2)','delimiter',',','-append');
        end
    end
    %fprintf('%f - %f\n',countEa,countNea);
end
